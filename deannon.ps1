...
function Write-GeneratedPairs {
    ...
    try {
        ...
        Set-Content -Path $AutoFile -Value $json
    }
    catch {
...
if ($configObject.generated_pairs_file) {
    ...
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($rawAuto)) {
            $autoEntries = @()
        } else {
            try {
                $autoEntries = ConvertFrom-Json -InputObject $rawAuto -Depth 32
            } catch {
                throw "Invalid JSON in generated entries file '$autoFilePath': $($_.Exception.Message)"
            }
            if (-not $autoEntries) { $autoEntries = @() }
        }
    } catch {
    ...
    foreach ($entry in $autoEntries) {
        ...
        $configObject.pairs.full = @($configObject.pairs.full + [pscustomobject]@{
            name = if ($entry.name) { $entry.name } else { Get-AutoFullName -Config $configObject -OriginalValue $entry.original }
            original = $entry.original
            anonymized = $entry.anonymized
        })
    }
    $configObject.generated_pairs_file = $autoFilePath
    $script:AutoGeneratedPairs = @($autoEntries)
}
$configChanged = $false
$stats = @()
