...
function Get-CountTrace {
    ...
    if ($Value -is [System.Collections.ICollection]) {
        Write-Verbose ("[count:{0}] target={1} type={2} mode=ICollection" -f $Id, $targetLabel, $typeName)
        try {
            $result = $Value.Count
        }
        catch {
            throw ("count:{0}: ICollection Count failed on '{1}' (type {2}): {3}" -f $Id, $targetLabel, $typeName, $_.Exception.Message)
        }
    ...
    if ($Value -is [System.Collections.IEnumerable]) {
        Write-Verbose ("[count:{0}] target={1} type={2} mode=IEnumerable" -f $Id, $targetLabel, $typeName)
        $count = 0
        foreach ($item in $Value) { $count++ }
        Write-Verbose ("[count:{0}] target={1} result={2}" -f $Id, $targetLabel, $count)
        return $count
    }

    Write-Verbose ("[count:{0}] target={1} type={2} mode=Property" -f $Id, $targetLabel, $typeName)
    try {
        $result = $Value.Count
        Write-Verbose ("[count:{0}] target={1} result={2}" -f $Id, $targetLabel, $result)
        return $result
...
function Get-RandomizeLengthValue {
    param(
        [string]$Value,
        [int]$Default = 8
    )
...
function Get-ConfigObject {
    ...
    $config = [pscustomobject]@{
        ...
        randomize_default_length = 8
        randomize_default_length_explicit = $false
        includes = @()
        section_order = @()
        extra_sections = @()
    }
    ...
    if ($sections.Keys -contains 'global') {
        ...
        if ($globalSection.Keys -contains 'randomize_default_length') {
            $config.randomize_default_length = Get-RandomizeLengthValue -Value $globalSection['randomize_default_length'] -Default 8
            $config.randomize_default_length_explicit = $true
        }
        elseif ($globalSection.Keys -contains 'randomize_default') {
            $config.randomize_default_length = Get-RandomizeLengthValue -Value $globalSection['randomize_default'] -Default 8
            $config.randomize_default_length_explicit = $true
        }
    }
    ...
    foreach ($sectionName in $sections.Keys) {
        ...
        elseif ($sectionName -like 'hint.*') {
            ...
            $disabled = Get-SectionDisabledFlag -Section $section
            $hasNextIndexKey = $section.Keys -contains 'next_index'
            $hasRandomizeKey = $section.Keys -contains 'randomize'
            $hasRandomLengthKey = $section.Keys -contains 'random_length'

            $randomizeValue = $null
            if ($hasRandomizeKey) {
                $randomizeValue = Get-RandomizeLengthValue -Value $section['randomize'] -Default $config.randomize_default_length
            }
            elseif ($hasRandomLengthKey) {
                $randomizeValue = Get-RandomizeLengthValue -Value $section['random_length'] -Default $config.randomize_default_length
            }
            elseif (-not $hasNextIndexKey) {
                $randomizeValue = $config.randomize_default_length
            }
            ...
            $hintObj = [pscustomobject]@{
                ...
                next_index = if ($hasNextIndexKey) { [int]$section['next_index'] } else { $null }
...
function Ensure-ConfigShape {
    ...
    if (-not $Config.PSObject.Properties.Match('randomize_default_length')) {
        $Config | Add-Member -NotePropertyName 'randomize_default_length' -NotePropertyValue 8
    }
    if (-not $Config.PSObject.Properties.Match('randomize_default_length_explicit')) {
        $Config | Add-Member -NotePropertyName 'randomize_default_length_explicit' -NotePropertyValue $false
    }
...
function Save-ConfigObject {
    ...
    $globalLines = New-Object System.Collections.Generic.List[string]
    if ($Config.generated_pairs_file) {
        $globalLines.Add("generated_entries_file=$($Config.generated_pairs_file)")
    }
    $globalRandomLength = if ($Config.PSObject.Properties.Match('randomize_default_length')) { [int]$Config.randomize_default_length } else { 8 }
    $globalRandomExplicit = if ($Config.PSObject.Properties.Match('randomize_default_length_explicit')) { [bool]$Config.randomize_default_length_explicit } else { $false }
    if ($globalRandomExplicit -or $globalRandomLength -ne 8) {
        $globalLines.Add("randomize_default_length=$globalRandomLength")
    }
    if ($globalLines.Count -gt 0) {
        $lines.Add('[global]')
        foreach ($entry in $globalLines) {
            $lines.Add($entry)
        }
        $lines.Add('')
    }
