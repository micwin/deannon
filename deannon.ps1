...
function Add-HintFullPair {
    ...
    $pair = [pscustomobject]@{
        ...
        wrap_prefix = $WrapPrefix
        wrap_postfix = $WrapPostfix
    }
...
function Write-GeneratedPairs {
    ...
    foreach ($entry in ($Entries | Sort-Object name)) {
        ...
        if ($entry.PSObject.Properties.Match('wrap_prefix') -and $entry.wrap_prefix) {
            $lines.Add("wrap_prefix=$($entry.wrap_prefix)")
        }
        if ($entry.PSObject.Properties.Match('wrap_postfix') -and $entry.wrap_postfix) {
            $lines.Add("wrap_postfix=$($entry.wrap_postfix)")
        }
        if ($entry.source) {
...
function Convert-HintAssignmentsToFull {
    ...
    foreach ($hint in $Config.pairs.hints) {
        ...
        foreach ($assignment in $hint.assignments) {
            ...
            if (-not $existing) {
                Add-HintFullPair -Config $Config -OriginalValue $assignment.original -AnonymizedValue $assignment.anonymized -SourceHint $hint.name -WrapPrefix $hint.wrap_prefix -WrapPostfix $hint.wrap_postfix | Out-Null
            }
        }
        $hint.assignments = @()
    }
    ...
    foreach ($sectionName in $sections.Keys) {
        if ($sectionName -like 'full.*') {
            ...
            $config.pairs.full += [pscustomobject]@{
                ...
                wrap_prefix = if ($section.Keys -contains 'wrap_prefix') { $section['wrap_prefix'] } else { $null }
                wrap_postfix = if ($section.Keys -contains 'wrap_postfix') { $section['wrap_postfix'] } else { $null }
            }
        }
        elseif ($sectionName -like 'hint.*') {
            ...
            $hintObj = [pscustomobject]@{
                ...
                random_length = if ($section.Keys -contains 'random_length') { [int]$section['random_length'] } else { $null }
...
function Ensure-ConfigShape {
    ...
    foreach ($pair in $Config.pairs.full) {
        if (-not $pair.PSObject.Properties.Match('wrap_prefix')) {
            $pair | Add-Member -NotePropertyName 'wrap_prefix' -NotePropertyValue $null
        }
        if (-not $pair.PSObject.Properties.Match('wrap_postfix')) {
            $pair | Add-Member -NotePropertyName 'wrap_postfix' -NotePropertyValue $null
        }
    }

    foreach ($hint in $Config.pairs.hints) {
        if (-not $hint.PSObject.Properties.Match('wrap_prefix')) {
            $hint | Add-Member -NotePropertyName 'wrap_prefix' -NotePropertyValue $null
        }
        if (-not $hint.PSObject.Properties.Match('wrap_postfix')) {
            $hint | Add-Member -NotePropertyName 'wrap_postfix' -NotePropertyValue $null
        }
        if (-not $hint.PSObject.Properties.Match('random_length')) {
            $hint | Add-Member -NotePropertyName 'random_length' -NotePropertyValue $null
        }
...
function Save-ConfigObject {
    ...
    foreach ($pair in $Config.pairs.full) {
        ...
        if ($pair.PSObject.Properties.Match('wrap_prefix') -and $pair.wrap_prefix) { $lines.Add("wrap_prefix=$($pair.wrap_prefix)") }
        if ($pair.PSObject.Properties.Match('wrap_postfix') -and $pair.wrap_postfix) { $lines.Add("wrap_postfix=$($pair.wrap_postfix)") }
        $lines.Add('')
    }
    ...
    foreach ($hint in $Config.pairs.hints) {
        ...
        if ($hint.PSObject.Properties.Match('random_length') -and $hint.random_length) {
            $lines.Add("random_length=$($hint.random_length)")
        } else {
            $widthValue = if ($hint.width) { $hint.width } else { 4 }
            $lines.Add("width=$widthValue")
        }
        if ($hint.PSObject.Properties.Match('random_charset') -and $hint.random_charset) {
            $lines.Add("random_charset=$($hint.random_charset)")
        }
        if (-not ($hint.PSObject.Properties.Match('random_length') -and $hint.random_length)) {
            $nextValue = if ($hint.next_index) { $hint.next_index } else { 1 }
            $lines.Add("next_index=$nextValue")
        }
        if ($hint.PSObject.Properties.Match('wrap_prefix') -and $hint.wrap_prefix) {
            $lines.Add("wrap_prefix=$($hint.wrap_prefix)")
        }
        if ($hint.PSObject.Properties.Match('wrap_postfix') -and $hint.wrap_postfix) {
            $lines.Add("wrap_postfix=$($hint.wrap_postfix)")
        }
        $lines.Add('')
    }
...
function Get-AnonymizedOutputValue {
    ...
    $prefix = if ($Pair.PSObject.Properties.Match('wrap_prefix')) { $Pair.wrap_prefix } else { $null }
    $postfix = if ($Pair.PSObject.Properties.Match('wrap_postfix')) { $Pair.wrap_postfix } else { $null }
    return Get-WrappedValue -Value $Pair.anonymized -Prefix $prefix -Postfix $postfix
}
...
function Apply-HintAnonymization {
    ...
    for ($i = 0; $i -lt $Hints.Count; $i++) {
        ...
        $useRandom = ($entry.PSObject.Properties.Match('random_length') -and $entry.random_length)
        if ($useRandom -and ($entry.PSObject.Properties.Match('next_index') -and $entry.next_index)) {
            throw "Hint '$($entry.name)' cannot define both random_length and next_index."
        }
        ...
        foreach ($originalValue in $uniqueOrder) {
            ...
            if ($existingPair) {
                ...
                $wrapPrefix = if ($existingPair.PSObject.Properties.Match('wrap_prefix')) { $existingPair.wrap_prefix } else { $null }
                $wrapPostfix = if ($existingPair.PSObject.Properties.Match('wrap_postfix')) { $existingPair.wrap_postfix } else { $null }
            } else {
                $wrapPrefix = $entry.wrap_prefix
                $wrapPostfix = $entry.wrap_postfix
                if ($useRandom) {
                    $charsetValue = if ($entry.PSObject.Properties.Match('random_charset') -and $entry.random_charset) { [string]$entry.random_charset } else { 'alnum' }
                    $baseRandom = New-RandomString -Length $entry.random_length -Charset $charsetValue
                    $prefixValue = if ($entry.PSObject.Properties.Match('prefix') -and $entry.prefix) { [string]$entry.prefix } else { '' }
                    $replacement = $prefixValue + $baseRandom
                } else {
                    $replacement = Get-NextHintValue -Hint $entry
                }
                Add-HintFullPair -Config $Config -OriginalValue $originalValue -AnonymizedValue $replacement -SourceHint $entry.name -WrapPrefix $wrapPrefix -WrapPostfix $wrapPostfix | Out-Null
                $ConfigChanged.Value = $true
            }

            $outputValue = Get-WrappedValue -Value $replacement -Prefix $wrapPrefix -Postfix $wrapPostfix
...
if ($configObject.generated_pairs_file) {
    ...
    foreach ($sectionName in $sections.Keys) {
        ...
        $entryObj = [pscustomobject]@{
            ...
            wrap_prefix = if ($section.Contains('wrap_prefix')) { $section['wrap_prefix'] } else { $null }
            wrap_postfix = if ($section.Contains('wrap_postfix')) { $section['wrap_postfix'] } else { $null }
        ...
        $configObject.pairs.full = @($configObject.pairs.full + [pscustomobject]@{
            ...
            wrap_prefix = $entryObj.wrap_prefix
            wrap_postfix = $entryObj.wrap_postfix
        })
    }
